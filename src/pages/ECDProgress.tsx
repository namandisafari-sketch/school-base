...
export default function ECDProgress() {
  ...
  const learningAreas = [
    t("ecd.literacy"), t("ecd.numeracy"), t("ecd.creativeArts"),
    t("ecd.socialSkills"), t("ecd.physical"), t("ecd.science"),
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        <div>
          <h1 className="page-title">{t("ecd.title")}</h1>
          <p className="page-description">{t("ecd.description")}</p>
        </div>
      </div>
      ...
      {!selectedClass ? (
        <Card><CardContent className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-sm text-muted-foreground">{t("ecd.selectClass")}</p>
        </CardContent></Card>
      ) : filteredStudents.length === 0 ? (
        <Card><CardContent className="flex flex-col items-center justify-center py-16">
          <Users className="mb-3 h-12 w-12 text-muted-foreground/30" />
          <p className="text-sm text-muted-foreground">{t("ecd.noStudents")} {selectedClass}</p>
        </CardContent></Card>
      ) : (
                ...
                <div className="space-y-1.5">
                  {learningAreas.slice(0, 3).map((area) => (
                    <div key={area} className="flex items-center justify-between text-xs">
                      ...
                      <Badge variant="outline" className="text-xs">{t("ecd.notRated")}</Badge>
                    </div>
                  ))}
                </div>
                <Button variant="outline" size="sm" className="mt-3 w-full text-xs">{t("ecd.rateProgress")}</Button>
              </CardContent>
            </Card>
          ))}
        </div>
